...
이 문서는 **파이프라인 코드를 수정하거나 기능을 추가하는 개발자**를 위한 안내입니다. 현재 구현된 설치·data preparation·전처리 사용법은 [README.md](README.md)를 참고하세요.

현재 config/data package는 구현되어 있고 trainer는 아직 구현 전입니다. 기능을 추가할 때
config와 문서에 정의된 데이터·모델·artifact 계약을 유지해야 합니다.
...
데이터 contract, config, split, parser, 전처리, Dataset 테스트를 모두 통과해야 합니다.
...
새 모델은 pipeline dataset이나 trainer를 직접 수정하지 않고 adapter contract로 연결합니다.

1. import 가능한 model factory를 구현합니다.
2. canonical batch를 model 입력으로 바꾸는 adapter를 구현합니다.
3. raw model 출력을 표준 output으로 변환합니다.
...
late fusion을 지원하는 branch는 embedding도 반환해야 합니다. 불확실성은 `gaze_xy`에 섞지 않고 별도 key로 반환합니다.

최소 model test는 다음을 포함합니다.
...
4. front-only, side-only, fusion metric을 모두 기록합니다.
...
- Git commit/dirty 상태, Python/PyTorch/CUDA 버전, resolved config hash, dataset/split hash를 MLflow에 기록합니다.
- raw 얼굴 image를 MLflow artifact로 자동 업로드하지 않습니다.

## Checkpoint 규칙

- 추론용 `best_weights.pt`에는 model `state_dict`만 저장합니다.
- 재개용 `last_checkpoint.pt`에는 model/optimizer/scheduler/scaler, epoch/step, best metric, RNG, config·dataset hash를 저장합니다.
