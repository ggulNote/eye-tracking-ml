...
@dataclass(frozen=True)
class RobustIntrinsicCalibration:
...
def calibrate_camera_intrinsics_robust(
    image_points: Sequence[np.ndarray],
    image_size: Tuple[int, int],
    spec: CheckerboardSpec,
    minimum_views: int,
    max_rms_error_px: float,
) -> RobustIntrinsicCalibration:
    """Reject the worst inconsistent view until RMS passes, preserving enough views."""

    if minimum_views < 3 or minimum_views > len(image_points):
        raise ValueError("minimum_views must be between 3 and the observation count.")
    if max_rms_error_px <= 0:
        raise ValueError("max_rms_error_px must be positive.")
    retained = list(range(len(image_points)))
    rejected = []
    while True:
        result = calibrate_camera_intrinsics(
            [image_points[index] for index in retained], image_size, spec
        )
        if result.rms_error_px <= max_rms_error_px or len(retained) <= minimum_views:
            return RobustIntrinsicCalibration(
                result=result,
                retained_indices=tuple(retained),
                rejected_indices=tuple(rejected),
            )
        worst_local_index = int(np.argmax(result.per_view_errors_px))
        rejected.append(retained.pop(worst_local_index))
...
def write_calibration_summary(
    ...
    rejected_capture_paths: Iterable[Path] = (),
) -> Path:
    resolved = path.expanduser().resolve()
    if resolved.exists():
    ...
    payload: Dict[str, Any] = {
        ...
        "rejected_captures": [str(value) for value in rejected_capture_paths],
    }
