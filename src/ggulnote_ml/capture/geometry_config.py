...
class CheckerboardCaptureConfig:
    ...
    camera_square_size_mm: Dict[str, float]
    camera_verification_source: Dict[str, str]
    required_captures: int
    minimum_captures: int
    max_rms_error_px: float

    def spec_for_camera(self, camera: str) -> CheckerboardSpec:
        try:
            square_size = self.camera_square_size_mm[camera]
        except KeyError as error:
            raise ValueError(
                "checkerboard.camera_measurements is missing %s." % camera
            ) from error
        return replace(self.spec, square_size_mm=square_size)

    def verification_source_for_camera(self, camera: str) -> str:
        try:
            return self.camera_verification_source[camera]
        except KeyError as error:
            raise ValueError(
                "checkerboard.camera_measurements is missing %s." % camera
            ) from error
...
def load_geometry_calibration_config(path: Path) -> GeometryCalibrationConfig:
    ...
    measurements = _mapping(board_raw, "camera_measurements")
    camera_square_sizes: Dict[str, float] = {}
    camera_verification_sources: Dict[str, str] = {}
    for camera in ("webcam", "phonecam"):
        measurement = _mapping(measurements, camera)
        square_size = float(
            _required(measurement, "square_size_mm", "checkerboard.camera_measurements.%s" % camera)
        )
        verification = str(
            _required(
                measurement,
                "verification_source",
                "checkerboard.camera_measurements.%s" % camera,
            )
        ).strip()
        if square_size <= 0:
            raise ValueError("Camera checkerboard square size must be positive.")
        if verification not in CHECKERBOARD_VERIFICATION_SOURCES:
            raise ValueError(
                "Camera checkerboard verification_source must be one of %s."
                % sorted(CHECKERBOARD_VERIFICATION_SOURCES)
            )
        camera_square_sizes[camera] = square_size
        camera_verification_sources[camera] = verification
    ...
    return GeometryCalibrationConfig(
        ...
        checkerboard=CheckerboardCaptureConfig(
            ...
            camera_square_size_mm=camera_square_sizes,
            camera_verification_source=camera_verification_sources,
