...
def run_training(config: PipelineConfig, project_root: Path) -> TrainingResult:
    ...
    with tracker:
        ...
        tracker.log_artifact(output_dir / "test_predictions.csv", artifact_path="evaluation")
        input_example = prepared.test.features.values[:2]
        output_example = predictions["test"][:2]
        tracker.log_model(
            model=model,
...
def _split_manifest(prepared: object) -> Dict[str, object]:
    ...
    for name in ("train", "validation", "test"):
        ...
        result[name] = {
            ...
            "participant_ids": sorted({sample.participant_id for sample in raw_split.samples}),
